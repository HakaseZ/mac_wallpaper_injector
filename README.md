...
- **注入视频壁纸**:选择本地视频,自动转码为 Aerial 合规格式(HEVC 10bit)、补丁 mov 结构、生成 16:10 缩略图,注入独立分类
- **分类管理**:同名分类复用,支持新建分类;注入资产完全独立(不混入系统 Landscape 资产)
- **选择壁纸**:模拟点击系统壁纸面板中的注入资产,立即生效
- **删除**:移除资产条目、视频、缩略图与空分类;被删资产若正被使用,自动回归系统默认动态壁纸
- **恢复基线**:一键清空全部注入资产,回归出厂动态壁纸(亮/暗切换)
...
1. 打开 MWIPanel.app,点击 **注入新壁纸** 选择视频文件
2. 注入完成后资产卡片出现在网格(分类分组)
3. 选中卡片点击 **设为壁纸**,系统立即切换
4. **删除选中** 移除资产;**恢复基线** 清空全部注入资产
